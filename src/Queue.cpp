...
// Queue<T>::pushBack(Cell<T>&& entry_ptr)
// {
//     size_t next_back;
//     size_t current_back;
...
//     buffer_[current_back].entry = std::move(entry_ptr.entry);
...
// //----------------------------------------------------------------------------------
// template<typename T> bool
// Queue<T>::popFront(Cell<T>& result_ptr)
// {
...
//     result_ptr.entry = std::move(buffer_[current_front].entry);
